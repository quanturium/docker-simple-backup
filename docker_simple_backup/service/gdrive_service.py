...
class GdriveService(ServiceInterface):
    def __init__(self, args):
        ...
        if args.service_gdrive_credentials is None:
            raise Exception(
                "Missing argument --service-gdrive-credentials for service: " + args.service)
        ...
        if args.service_gdrive_folder is None:
            raise Exception("Missing argument --service-gdrive-folder for service: " + args.service)

    def copy_archive(self, archive: Path):
        gdrive = self.get_gdrive()
        print("Uploading archive to gdrive...")
        file_id = self.upload_archive(
            gdrive, self.args.service_gdrive_folder, archive)
    ...
    def remove_old_archives(self, max_count: int):
        ...
        result = list(
            filter(lambda x: x['name'].endswith(".zip"), result))
    ...
    def upload_archive(self, gdrive, folder_id, archive: Path):
        ...
        file = gdrive.files().create(body=file_metadata,
                                     media_body=media, fields='name,id').execute()
        file_id = file.get('id')
        return file_id
